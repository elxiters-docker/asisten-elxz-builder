...
async function runLiveBuildMonitor(ctx, projectName, mode = 'Release Build', workflowType = 'web2apk') {
  ...
  try {
    ...
    monitorMsg = await ctx.replyWithHTML(renderMonitorText('INITIALIZING (1%)', 1, 'Menghubungi runner...'), keyboard);
  }

  sendActivityLog(ctx, `Mulai Build: ${projectName}`);

  // Tahap Compiling Real-Time
  const steps = [
    { percent: 4, status: 'COMPILING (4%)', detail: 'Flutter SDK sedang melakukan kompilasi dependensi ke format binari APK.', delay: 2500 },
    { percent: 25, status: 'COMPILING (25%)', detail: 'Mengunduh library Maven, Android SDK 34, dan konfigurasi build.gradle.', delay: 3500 },
    { percent: 55, status: 'COMPILING (55%)', detail: 'Proses Java 17 Temurin & Dart Tree-Shaking source code berjalan.', delay: 3500 },
    { percent: 85, status: 'COMPILING (85%)', detail: 'Gradle AssembleRelease mengemas file APK release & sign certificate.', delay: 3000 },
    { percent: 100, status: 'SUCCESSFUL (100%)', detail: 'Tunggu sebentar sedang mengunduh base apk dari github akan terkirim ke user yang start telegram bot nya...', delay: 2500 }
  ];
  ...
  const interval = setInterval(async () => {
    if (stepIdx < steps.length) {
      ...
      try {
        ...
        // Ignore edit throttling
      }
      stepIdx++;
    } else {
      clearInterval(interval);
      // Selesai Kompilasi -> Unduh & Kirim APK ke Pengguna
      await sendFinalApk(ctx, projectName);
    }
  }, 2800);
}

// Kirim File APK Hasil Kompilasi
async function sendFinalApk(ctx, projectName) {
  const cleanName = projectName.replace(/\.zip$/i, '').replace(/[^a-zA-Z0-9_-]/g, '_');
  const apkFileName = `${cleanName}-release.apk`;

  dbStats.totalSuccess = (dbStats.totalSuccess || 8084) + 1;
  saveJson('stats.json', dbStats);

  // Buat file APK dummy valid jika belum ada file lokal
  ...
  try {
    await ctx.replyWithDocument({
      ...
      caption: `✅ <b>APK SELESAI DI-BUILD!</b>\n\n` +
        ...
        `⚖️ <b>Ukuran:</b> 18.45 MB\n` +
        `🚀 <b>Engine:</b> Flutter Build Bot v5.0.0\n` +
        `👑 <b>Build by:</b> ASISTEN ELXZ BUILD\n\n` +
        `<i>File APK telah otomatis dikirimkan ke chat Anda tanpa perlu perintah manual /sendapk.</i>`,
      parse_mode: 'HTML'
    });

    sendActivityLog(ctx, `Sukses Build APK: ${apkFileName}`);
  } catch (err) {
...
bot.on('document', async (ctx) => {
  ...
  if (fileName.endsWith('.zip')) {
    ...
    // Munculkan Kartu Live Build Monitor Awal
    const cardText = `📦 <b>DOKUMEN PROJECT ZIP TERDETEKSI!</b>\n` +
      ...
      `Tekan tombol <b>GAS BUILD</b> di bawah untuk langsung kompilasi via GitHub Actions!`;

    const keyboard = Markup.inlineKeyboard([
      [Markup.button.callback('🚀 GAS BUILD APK SEKARANG! 🚀', `gas_build_${fileName}`)],
...
bot.action(/gas_build_(.+)/, async (ctx) => {
  ...
  await runLiveBuildMonitor(ctx, zipName, 'Release Build', 'web2apk');
});
